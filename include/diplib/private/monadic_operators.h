...
/// Only defined for floating-point types, the output is the same type.
DIP_MONADIC_OPERATOR_FLOAT( Fraction, []( auto its ) { return dipm_Fraction( *its[ 0 ] ); }, DataType::Class_Float, 1 )

/// \brief Computes the reciprocal of each sample: out = in == 0 ? 0 : 1/in.
DIP_MONADIC_OPERATOR_FLEX( Reciprocal, []( auto its ) { return dipm_Reciprocal( *its[ 0 ] ); }, DataType::Class_NonBinary, 1 )
...
DIP_MONADIC_OPERATOR_BIN( IsNotANumber, []( auto in ) { return dipm_IsNaN( in ); }, DataType::Class_Flex, false )

/// \brief True for each pixel that is positive or negative infinity.
DIP_MONADIC_OPERATOR_BIN( IsInfinite, []( auto in ) { return dipm_IsInf( in ); }, DataType::Class_Flex, false )

/// \brief True for each pixel that is not NaN nor infinity.
DIP_MONADIC_OPERATOR_BIN( IsFinite, []( auto in ) { return dipm_IsFinite( in ); }, DataType::Class_Flex, true )
